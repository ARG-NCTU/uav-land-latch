...
import csv
from gymnasium_vrx.utils.uavlander_drone import uavROSConnector
from gymnasium_vrx.utils.uavlander_wamv import wamvGazeboROSConnector
from gymnasium_vrx.utils.uavlander_gazebo import GazeboROSConnector
...
############################################################################################################
    ...
    def _get_reward(self, action, step):
        ...
        output = "\rstep:{:4d}, actions[{},{},{},{}], r:{:.2f}, h:{}, hight_score:{}, dist_rew:{}, radius_bonus:{}, vz_punish:{}, reward:{}, landing{}".format(
            self.counter_step,
            ...
            r,
            " {:3.2f}".format(h) if h >= 0 else "{:3.2f}".format(h),
            " {:3.2f}".format(hight_score) if hight_score >= 0 else "{:3.2f}".format(hight_score),
            " {:3.2f}".format(dist_rew) if dist_rew >= 0 else "{:3.2f}".format(dist_rew),
            " {:3.2f}".format(radius_bonus) if radius_bonus >= 0 else "{:3.2f}".format(radius_bonus),
            " {:3.2f}".format(vz_punish) if vz_punish >= 0 else "{:3.2f}".format(vz_punish),
            " {:4.2f}".format(reward) if reward >= 0 else "{:4.2f}".format(reward),
            "---Success---" if success else "---Faild:{:1d}---".format(self.land_cnt)
        )
