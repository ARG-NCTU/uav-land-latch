...
############################################################################################################
    ...
    def _get_reward(self, action, step):
        ...
        hight_score =       0 +                 reward_max*u(goal_z_tol-abs(h)) + reward_max*curve(h*0.8)*u(h-goal_z_tol)
        #------------- under platform ------------ between platform +- 5cm ------------ above platform
        reward = hight_score*(1-u(h))+curve(abs(r))*u(h)
    ...
    def _get_observation(self):
        # self.prev_distance = np.linalg.norm(self.range - self.goal_range)
