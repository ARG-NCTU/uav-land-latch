...
class DroneRangeTrackV2(gym.Env):
    ...
    def step(self, action):
        ...
        if dist_xyz < 0.2 and dist_yaw < 0.1745329:
            reward += 20.0
            terminated = True
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.respawn_range = 10.0
        self.respawn_yaw = np.pi
        self.respawn_offset = 10.0

        self.respawn_range = [
            (-self.respawn_range, self.respawn_range),
            (-self.respawn_range, self.respawn_range),
            (-self.respawn_range, self.respawn_range),
            (-self.respawn_yaw, self.respawn_yaw),
        ]

        self.goal_pose = np.array([self.np_random.uniform(low, high) for low, high in self.respawn_range])
        offset = self.np_random.uniform(-self.respawn_offset, self.respawn_offset, size=3)
        self.drone_pose[0:3] = self.goal_pose[0:3] + offset
        self.drone_pose[3] = self.np_random.uniform(-self.respawn_yaw, self.respawn_yaw)
    ...
    def pose_to_signature(self, pose: np.ndarray) -> np.ndarray:
        ...
        distances = np.linalg.norm(uwb_tag_pos[:, None, :] - self.uwb_anchor_pos, axis=2)
        signature = distances.flatten() / 50.0
        return signature

    def body_pose_to_uwb_tag_pos(self, pose: np.ndarray) -> np.ndarray:
        # pose is the center of the drone in the world frame
        x, y, z, yaw = pose
        cos_yaw, sin_yaw = np.cos(yaw), np.sin(yaw)
