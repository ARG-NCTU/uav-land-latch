...
class LocailizationGPSImu():
    def __init__(self):
        ...
        self.ns = rospy.get_param('~namespace',"wamv")
        self.location_x = 0
        self.location_y = 0
        ...
        self.pub_vector = rospy.Publisher("~vis_vector", Marker, queue_size=1)
        

        # Subscriber
        sub_imu = message_filters.Subscriber("sensors/imu/imu/data", Imu)
        sub_gps = message_filters.Subscriber("sensors/gps/gps/fix", NavSatFix)
    ...
    def kalman_filter(self):
        ...
        time_diff = (self.time.to_sec()-self.prior_time.to_sec())
        vx = (posterior_x.mean() - self.prior_pose.position.x.mean()) / time_diff
        vy = (posterior_y.mean() - self.prior_pose.position.y.mean()) / time_diff
        self.prior_roll = posterior_roll
        self.prior_pitch = posterior_pitch
        self.prior_yaw = posterior_yaw
        self.prior_time = self.time
        ...
        self.posestamped.header = self.odometry.header
        self.posestamped.pose = self.odometry.pose.pose
        self.pub_pos.publish(self.posestamped)
        ...
        self.br.sendTransform((self.odometry.pose.pose.position.x, \
                        ...
                        rospy.Time.now(),"/"+self.ns+"/base_link","/map")
